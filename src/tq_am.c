...
#define TQ_MAX_SUPPORTED_KEY_ATTRIBUTES (1 + TQ_MAX_STORED_METADATA_ATTRIBUTES)
#define TQ_FILTER_EQ_STRATEGY 1
#define TQ_FAST_LANE_NORMALIZATION_TOLERANCE 0.10f
...
typedef struct TqBuildState
{
	...
	uint8_t	   *collected_exact_keys;
	uint8_t	   *collected_metadata_values;
	uint16_t   *collected_metadata_nullmasks;
	uint8_t	   *packed_code;
	size_t		vector_count;
	size_t		vector_capacity;
	double		index_tuples;
	size_t		exact_key_bytes;
	uint16_t	metadata_attribute_count;
	uint16_t	key_filter_attribute_count;
	TqMetadataAttrDesc metadata_descs[TQ_MAX_STORED_METADATA_ATTRIBUTES];
} TqBuildState;
...
typedef struct TqScanOpaque
{
	...
	TqMetadataAttrDesc metadata_descs[TQ_MAX_STORED_METADATA_ATTRIBUTES];
	TqMetadataFilterClause filter_clauses[TQ_MAX_STORED_METADATA_ATTRIBUTES];
	bool		prepared;
} TqScanOpaque;
...
static float
tq_vector_norm_squared(const float *values, uint32_t dimension)
{
	float sum = 0.0f;
	uint32_t index = 0;
...
static void
tq_maybe_normalize_fast_lane_vector(float *values,
									 uint32_t dimension,
									 const char *label)
{
	...
	norm_squared = tq_vector_norm_squared(values, dimension);
	if (norm_squared <= 0.0f)
		ereport(ERROR,
				(errcode(ERRCODE_DATA_EXCEPTION),
				 errmsg("turboquant normalized indexes require unit-norm %s", label)));

	norm = sqrtf(norm_squared);
	if (fabsf(norm - 1.0f) > TQ_FAST_LANE_NORMALIZATION_TOLERANCE)
		ereport(ERROR,
				(errcode(ERRCODE_DATA_EXCEPTION),
				 errmsg("turboquant normalized indexes require unit-norm %s", label)));

	if (fabsf(norm - 1.0f) > 1.0e-6f)
	{
		for (index = 0; index < dimension; index++)
			values[index] /= norm;
	}
...
static void
tq_collect_metadata_values(Relation index_relation,
						   Datum *values,
						   bool *isnull,
						   uint8_t *metadata_values,
						   uint16_t *metadata_nullmask,
						   TqMetadataAttrDesc *descs,
						   uint16_t metadata_attribute_count)
{
	uint16_t attribute_index = 0;
	size_t metadata_block_bytes = tq_metadata_block_bytes(metadata_attribute_count);
	char error_buf[256];

	if (metadata_values != NULL && metadata_block_bytes > 0)
		memset(metadata_values, 0, metadata_block_bytes);
	if (metadata_nullmask != NULL)
		*metadata_nullmask = 0;
	if (descs != NULL)
		memset(descs, 0, sizeof(TqMetadataAttrDesc) * (size_t) metadata_attribute_count);

	for (attribute_index = 0; attribute_index < metadata_attribute_count; attribute_index++)
	{
		Form_pg_attribute attribute = TupleDescAttr(index_relation->rd_att, attribute_index + 1);
		TqMetadataAttrDesc desc;

		memset(&desc, 0, sizeof(desc));
		if (!tq_metadata_attr_desc_init(attribute->atttypid,
										&desc,
										error_buf,
										sizeof(error_buf)))
		...
		if (descs != NULL)
			descs[attribute_index] = desc;

		if (isnull[attribute_index + 1])
		{
			if (metadata_nullmask != NULL)
				*metadata_nullmask |= (uint16_t) (1u << attribute_index);
			continue;
		}

		if (metadata_values != NULL
			&& !tq_metadata_encode_datum(desc.kind,
										 values[attribute_index + 1],
										 metadata_values + ((size_t) attribute_index * TQ_METADATA_SLOT_BYTES),
										 error_buf,
										 sizeof(error_buf)))
			elog(ERROR, "%s", error_buf);
	}
...
static uint16_t
tq_relation_metadata_attribute_count(Relation index_relation,
									 TqMetadataAttrDesc *descs,
									 char *errmsg,
									 size_t errmsg_len)
{
	...
	if (attribute_count <= 1)
		return 0;
	if (attribute_count - 1 > TQ_MAX_STORED_METADATA_ATTRIBUTES)
	{
		snprintf(errmsg, errmsg_len,
				 "turboquant supports at most %d stored fixed-width metadata attributes after the embedding key",
				 TQ_MAX_STORED_METADATA_ATTRIBUTES);
		return UINT16_MAX;
	}

	for (attno = 1; attno < attribute_count; attno++)
	{
		Oid typid = TupleDescAttr(index_relation->rd_att, attno)->atttypid;

		if (descs != NULL
			&& !tq_metadata_attr_desc_init(typid,
										   &descs[attno - 1],
										   errmsg,
										   errmsg_len))
			return UINT16_MAX;
		if (descs == NULL
			&& !tq_metadata_kind_from_typid(typid, NULL, errmsg, errmsg_len))
		{
			snprintf(errmsg, errmsg_len,
					 "turboquant metadata columns after the embedding key must all be fixed-width bool/int2/int4/int8/date/timestamptz/uuid");
			return UINT16_MAX;
		}
	}
									   ...
									   size_t errmsg_len)
{
	...
	{
		snprintf(errmsg, errmsg_len,
				 "turboquant supports at most one embedding key plus %d fixed-width filter keys",
				 TQ_MAX_STORED_METADATA_ATTRIBUTES);
		return UINT16_MAX;
	}
	...
	{
		if (!tq_metadata_kind_from_typid(TupleDescAttr(index_relation->rd_att, attno)->atttypid,
										 NULL,
										 errmsg,
										 errmsg_len))
		{
			snprintf(errmsg, errmsg_len,
					 "turboquant filter key columns after the embedding key must be fixed-width bool/int2/int4/int8/date/timestamptz/uuid");
			return UINT16_MAX;
		}
	}
...
static void
tq_validate_index_schema(Relation index_relation)
{
	int attribute_count = 0;
	int key_attribute_count = 0;
	...
	if (index_relation == NULL)
		elog(ERROR, "turboquant index relation must be non-null");
	...
	if (key_attribute_count < 1 || key_attribute_count > TQ_MAX_SUPPORTED_KEY_ATTRIBUTES)
		ereport(ERROR,
				(errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
				 errmsg("turboquant supports one embedding key column plus compact fixed-width filter keys")));

	if (key_attribute_count >= 1)
	{
		Oid vector_type = TupleDescAttr(index_relation->rd_att, 0)->atttypid;
		TqVectorInputKind input_kind;

		if (!tq_vector_input_kind_from_typid(vector_type, &input_kind,
	...
	metadata_attribute_count = tq_relation_metadata_attribute_count(index_relation,
																   NULL,
																   error_buf,
																   sizeof(error_buf));
	key_filter_attribute_count = tq_relation_filter_key_attribute_count(index_relation,
																	 error_buf,
																	 sizeof(error_buf));
	if (metadata_attribute_count == UINT16_MAX || key_filter_attribute_count == UINT16_MAX)
		ereport(ERROR,
				(errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
				 errmsg("%s", error_buf)));

	if (attribute_count > key_attribute_count
		&& metadata_attribute_count == 0)
		ereport(ERROR,
				(errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
				 errmsg("turboquant INCLUDE columns must be compact fixed-width metadata columns")));
}

static void tq_load_option_config(Relation index_relation, TqOptionConfig *config);
static uint16_t tq_relation_metadata_attribute_count(Relation index_relation,
													 TqMetadataAttrDesc *descs,
													 char *errmsg,
													 size_t errmsg_len);
...
static bool tq_refresh_batch_page_summary(Relation index_relation,
										  ...
										  char *errmsg,
										  size_t errmsg_len);
static bool tq_batch_summary_capture_metadata_synopsis(const void *page,
														 size_t page_size,
														 const TqMetadataAttrDesc *metadata_descs,
														 TqBatchPageSummary *summary,
														 char *errmsg,
														 size_t errmsg_len);
...
static void tq_truncate_reusable_tail_pages(Relation index_relation,
											 ...
											 char *errmsg,
											 size_t errmsg_len);
static void tq_update_exact_key_page_next_block(Relation index_relation,
												BlockNumber block_number,
												BlockNumber next_block);
static void tq_append_exact_key(Relation index_relation,
								TqMetaPageFields *meta_fields,
								const uint8_t *exact_key_bytes,
								size_t exact_key_len,
								TqExactKeyRef *exact_key_ref);
static bool tq_read_exact_key_bytes(Relation index_relation,
									const TqExactKeyRef *exact_key_ref,
									uint8_t *exact_key_bytes,
									size_t exact_key_len,
									char *errmsg,
									size_t errmsg_len);
...
static void tq_append_delta_packed_tuple_locked(Relation index_relation,
												TqMetaPageFields *meta_fields,
												const TqProdCodecConfig *prod_config,
												uint32_t list_id,
												const TqTid *heap_tid,
...
static bool tq_collect_bounded_pages_for_list(Relation index_relation,
											  ...
											  char *errmsg,
											  size_t errmsg_len);
static bool tq_estimate_filtered_pages_for_list(Relation index_relation,
												 const TqListDirEntry *entry,
												 TqScanOpaque *opaque,
												 uint32_t *estimated_live_count,
												 uint32_t *estimated_page_count,
												 char *errmsg,
												 size_t errmsg_len);
...
tq_reset_scan_opaque(TqScanOpaque *opaque)
{
	...
	opaque->source_dimension = 0;
	opaque->exact_key_bytes = 0;
	opaque->shadow_decode_diagnostics = false;
	opaque->metadata_attribute_count = 0;
	opaque->key_filter_attribute_count = 0;
	opaque->filter_clause_count = 0;
	memset(opaque->metadata_descs, 0, sizeof(opaque->metadata_descs));
...
tq_buildstate_reset(TqBuildState *state)
{
	...
	if (state->collected_exact_keys != NULL)
		pfree(state->collected_exact_keys);
	if (state->collected_metadata_values != NULL)
		pfree(state->collected_metadata_values);
	if (state->collected_metadata_nullmasks != NULL)
		pfree(state->collected_metadata_nullmasks);
...
tq_buildstate_initialize(TqBuildState *state, uint32_t dimension)
{
	...
	state->exact_key_bytes = tq_vector_storage_size(state->input_kind, dimension);
	...
	lane_config.tid_bytes = TQ_TID_BYTES
		+ (int) tq_metadata_block_bytes(state->metadata_attribute_count)
		+ (int) sizeof(TqExactKeyRef);
	...
	state->meta_fields.delta_head_block = TQ_INVALID_BLOCK_NUMBER;
	state->meta_fields.delta_tail_block = TQ_INVALID_BLOCK_NUMBER;
	state->meta_fields.delta_live_count = 0;
	state->meta_fields.delta_batch_page_count = 0;
	state->meta_fields.exact_key_head_block = TQ_INVALID_BLOCK_NUMBER;
	state->meta_fields.exact_key_tail_block = TQ_INVALID_BLOCK_NUMBER;
	state->meta_fields.exact_key_page_count = 0;
							 ...
							 uint16_t metadata_nullmask)
{
	...
	TqTid		page_tid;
	size_t		metadata_block_bytes = tq_metadata_block_bytes(state->metadata_attribute_count);
	...
	if (required_count > state->vector_capacity)
	{
		...
		if (state->exact_key_bytes > 0)
		{
			size_t exact_key_storage_bytes = state->exact_key_bytes * new_capacity;

			if (state->collected_exact_keys == NULL)
				state->collected_exact_keys = (uint8_t *) palloc0(exact_key_storage_bytes);
			else
				state->collected_exact_keys = (uint8_t *) repalloc(state->collected_exact_keys,
																 exact_key_storage_bytes);
		}

		if (metadata_block_bytes > 0)
		{
			size_t byte_count = metadata_block_bytes * new_capacity;

			if (state->collected_metadata_values == NULL)
				state->collected_metadata_values = (uint8_t *) palloc0(byte_count);
			else
				state->collected_metadata_values = (uint8_t *) repalloc(state->collected_metadata_values,
																	   byte_count);
			if (state->collected_metadata_nullmasks == NULL)
				state->collected_metadata_nullmasks = (uint16_t *) palloc0(sizeof(uint16_t) * new_capacity);
			else
				state->collected_metadata_nullmasks = (uint16_t *) repalloc(state->collected_metadata_nullmasks,
																			sizeof(uint16_t) * new_capacity);
		}
	...
	if (state->exact_key_bytes > 0)
	{
		uint8_t *dst = state->collected_exact_keys + (state->vector_count * state->exact_key_bytes);

		memset(dst, 0, state->exact_key_bytes);
		if (exact_key_bytes != NULL)
			memcpy(dst, exact_key_bytes, state->exact_key_bytes);
	}
	if (metadata_block_bytes > 0)
	{
		uint8_t *dst = state->collected_metadata_values
			+ (state->vector_count * metadata_block_bytes);

		memset(dst, 0, metadata_block_bytes);
		if (metadata_values != NULL)
			memcpy(dst, metadata_values, metadata_block_bytes);
		state->collected_metadata_nullmasks[state->vector_count] = metadata_nullmask;
	}
...
static void
tq_update_exact_key_page_next_block(Relation index_relation,
									BlockNumber block_number,
									BlockNumber next_block)
{
	Buffer buffer;
	...
	buffer = ReadBufferExtended(index_relation, MAIN_FORKNUM, block_number, RBM_NORMAL, NULL);
	LockBuffer(buffer, BUFFER_LOCK_EXCLUSIVE);
	if (!tq_wal_set_exact_key_next_block(index_relation,
										 buffer,
										 next_block,
										 error_buf,
										 sizeof(error_buf)))
...
static void
tq_append_exact_key(Relation index_relation,
					TqMetaPageFields *meta_fields,
					const uint8_t *exact_key_bytes,
					size_t exact_key_len,
					TqExactKeyRef *exact_key_ref)
{
	...
	BlockNumber new_block = TQ_INVALID_BLOCK_NUMBER;
	char error_buf[256];

	if (meta_fields == NULL || exact_key_bytes == NULL || exact_key_ref == NULL || exact_key_len == 0)
		elog(ERROR, "turboquant exact-key append requires meta, bytes, and ref outputs");

	if (meta_fields->exact_key_tail_block != TQ_INVALID_BLOCK_NUMBER)
	{
		TqExactKeyPageHeaderView header;

		buffer = ReadBufferExtended(index_relation,
									MAIN_FORKNUM,
									meta_fields->exact_key_tail_block,
									RBM_NORMAL,
									NULL);
		memset(&header, 0, sizeof(header));
		if (!tq_exact_key_page_read_header(tq_page_payload(BufferGetPage(buffer)),
										   tq_page_payload_size(BufferGetPage(buffer)),
										   &header,
										   error_buf,
										   sizeof(error_buf)))
		...
		if (header.key_bytes != exact_key_len || header.entry_count >= header.entry_capacity)
		{
			ReleaseBuffer(buffer);
	...
	{
		uint16_t entry_capacity = tq_exact_key_page_capacity(tq_relation_payload_size(),
															 (uint32_t) exact_key_len);

		if (entry_capacity == 0)
			elog(ERROR, "turboquant exact-key payload does not fit on an index page");

		buffer = ReadBufferExtended(index_relation, MAIN_FORKNUM, P_NEW, RBM_NORMAL, NULL);
		LockBuffer(buffer, BUFFER_LOCK_EXCLUSIVE);
		new_block = BufferGetBlockNumber(buffer);
		if (!tq_wal_init_exact_key_page(index_relation,
										buffer,
										(uint32_t) exact_key_len,
										entry_capacity,
		...
		if (meta_fields->exact_key_tail_block != TQ_INVALID_BLOCK_NUMBER)
			tq_update_exact_key_page_next_block(index_relation,
												meta_fields->exact_key_tail_block,
												new_block);
		if (meta_fields->exact_key_head_block == TQ_INVALID_BLOCK_NUMBER)
			meta_fields->exact_key_head_block = new_block;
		meta_fields->exact_key_tail_block = new_block;
		meta_fields->exact_key_page_count += 1;
	}

	LockBuffer(buffer, BUFFER_LOCK_EXCLUSIVE);
	if (!tq_wal_append_exact_key(index_relation,
								 buffer,
								 exact_key_bytes,
								 exact_key_len,
								 &exact_key_ref->entry_index,
								 error_buf,
								 sizeof(error_buf)))
	{
		LockBuffer(buffer, BUFFER_LOCK_UNLOCK);
	...
	exact_key_ref->block_number = BufferGetBlockNumber(buffer);
	LockBuffer(buffer, BUFFER_LOCK_UNLOCK);
	ReleaseBuffer(buffer);
}

static bool
tq_read_exact_key_bytes(Relation index_relation,
						const TqExactKeyRef *exact_key_ref,
						uint8_t *exact_key_bytes,
						size_t exact_key_len,
						char *errmsg,
						size_t errmsg_len)
{
	...
	if (index_relation == NULL || exact_key_ref == NULL || exact_key_bytes == NULL || exact_key_len == 0)
	{
		snprintf(errmsg, errmsg_len,
				 "invalid turboquant exact-key read: relation, ref, bytes, and length must be non-null");
		return false;
	}
	if (exact_key_ref->block_number == TQ_INVALID_BLOCK_NUMBER || exact_key_ref->entry_index == UINT16_MAX)
	{
		snprintf(errmsg, errmsg_len,
				 "invalid turboquant exact-key read: candidate is missing exact-key storage");
		return false;
	}

	buffer = ReadBufferExtended(index_relation,
								MAIN_FORKNUM,
								exact_key_ref->block_number,
								RBM_NORMAL,
								NULL);
	ok = tq_exact_key_page_get_entry(tq_page_payload(BufferGetPage(buffer)),
									 tq_page_payload_size(BufferGetPage(buffer)),
									 exact_key_ref->entry_index,
									 exact_key_bytes,
									 exact_key_len,
...
static bool
tq_batch_summary_capture_metadata_synopsis(const void *page,
										   size_t page_size,
										   const TqMetadataAttrDesc *metadata_descs,
										   TqBatchPageSummary *summary,
										   char *errmsg,
										   size_t errmsg_len)
{
	...
	{
		snprintf(errmsg, errmsg_len,
				 "invalid turboquant batch summary capture: page and summary must be non-null");
		return false;
	}

	summary->null_any_mask = 0;
	summary->null_all_mask = 0;
	summary->all_same_mask = 0;
	memset(summary->same_values, 0, sizeof(summary->same_values));
	memset(summary->min_values, 0, sizeof(summary->min_values));
	memset(summary->max_values, 0, sizeof(summary->max_values));
	memset(&header, 0, sizeof(header));
	memset(metadata_values, 0, sizeof(metadata_values));

	if (!tq_batch_page_read_header(page, page_size, &header, errmsg, errmsg_len)
		|| !tq_batch_page_get_metadata_attribute_count(page,
													   page_size,
	...
	if (metadata_attribute_count == 0 || header.live_count == 0)
		return true;
	if (metadata_descs == NULL)
	{
		snprintf(errmsg, errmsg_len,
				 "invalid turboquant batch summary capture: metadata descriptors are required");
		return false;
	}

	if (!tq_batch_page_next_live_lane(page, page_size, -1, &lane_index, errmsg, errmsg_len))
		return true;
	...
	do
	{
		...
		for (attr_index = 0; attr_index < metadata_attribute_count; attr_index++)
		{
			const uint16_t bit = (uint16_t) (1u << attr_index);
			const uint8_t *value = metadata_values
				+ ((size_t) attr_index * (size_t) TQ_METADATA_SLOT_BYTES);

			if ((metadata_nullmask & bit) != 0)
			{
				summary->null_any_mask |= bit;
				continue;
			}

			if ((seen_nonnull_mask & bit) == 0)
			{
				seen_nonnull_mask |= bit;
				summary->all_same_mask |= bit;
				memcpy(summary->same_values[attr_index], value, TQ_METADATA_SLOT_BYTES);
				memcpy(summary->min_values[attr_index], value, TQ_METADATA_SLOT_BYTES);
				memcpy(summary->max_values[attr_index], value, TQ_METADATA_SLOT_BYTES);
				continue;
			}

			{
				...
				if (!tq_metadata_slot_compare(metadata_descs[attr_index].kind,
											  value,
											  summary->same_values[attr_index],
											  &cmp,
											  errmsg,
											  errmsg_len))
					return false;
				if (cmp != 0)
					summary->all_same_mask &= (uint16_t) ~bit;

				if (!tq_metadata_slot_compare(metadata_descs[attr_index].kind,
											  value,
											  summary->min_values[attr_index],
											  &cmp,
											  errmsg,
											  errmsg_len))
					return false;
	...
	while (tq_batch_page_next_live_lane(page, page_size, (int) lane_index, &lane_index,
										errmsg, errmsg_len));
								   ...
								   size_t errmsg_len)
{
	...
	if (!tq_batch_page_read_header(page, page_size, &header, errmsg, errmsg_len)
		...
		|| !tq_batch_summary_capture_metadata_synopsis(page,
													   page_size,
													   metadata_descs,
													   summary,
													   errmsg,
							  ...
							  size_t errmsg_len)
{
	...
	memset(metadata_descs, 0, sizeof(metadata_descs));
	metadata_attribute_count = tq_relation_metadata_attribute_count(index_relation,
																   metadata_descs,
																   errmsg,
																   errmsg_len);
	if (metadata_attribute_count == UINT16_MAX)
		return false;
	...
	while (block_number != TQ_INVALID_BLOCK_NUMBER && processed_pages < entry->batch_page_count)
	{
		...
		if (!tq_batch_page_read_header(tq_page_payload(batch_page), tq_page_payload_size(batch_page),
			...
			|| !tq_capture_batch_page_side_summary(tq_page_payload(batch_page),
												   ...
												   (uint32_t) layout.total_bytes,
												   metadata_attribute_count > 0 ? metadata_descs : NULL,
												   &summary,
												   representative_code,
												   errmsg,
												   errmsg_len))
	...
	for (list_id = 0; list_id < effective_list_count; list_id++)
	{
		...
		for (vector_index = 0; vector_index < state->vector_count; vector_index++)
		{
			...
			if (!BufferIsValid(current_buffer) || lanes_used >= state->lane_count)
			{
				...
				params.metadata_attribute_count = state->metadata_attribute_count;
				if (tq_prod_lut16_is_supported(&state->prod_config, NULL, 0)
					&& tq_batch_page_can_fit_soa_with_metadata(tq_relation_payload_size(),
															  params.lane_count,
															  state->prod_config.dimension,
															  params.code_bytes,
															  state->metadata_attribute_count))
			...
			{
				size_t metadata_block_bytes = tq_metadata_block_bytes(state->metadata_attribute_count);
				const uint8_t *row_exact_key_bytes = state->exact_key_bytes > 0
					? state->collected_exact_keys + (vector_index * state->exact_key_bytes)
					: NULL;
				const uint8_t *row_metadata_values = metadata_block_bytes > 0
					? state->collected_metadata_values + (vector_index * metadata_block_bytes)
					: NULL;
				uint16_t row_metadata_nullmask = state->metadata_attribute_count > 0
					? state->collected_metadata_nullmasks[vector_index]
					: 0;
				TqExactKeyRef exact_key_ref;

				exact_key_ref.block_number = TQ_INVALID_BLOCK_NUMBER;
				exact_key_ref.entry_index = UINT16_MAX;
				tq_append_exact_key(state->index_relation,
									&state->meta_fields,
									row_exact_key_bytes,
									state->exact_key_bytes,
									&exact_key_ref);
				  ...
				  bool *isnull, bool tupleIsAlive, void *stateptr)
{
	...
	if (state->option_config.normalized)
		tq_maybe_normalize_fast_lane_vector(state->source_values,
											state->source_dimension,
											"embeddings");

	exact_key_bytes = (uint8_t *) palloc(state->exact_key_bytes);
	if (!tq_vector_copy_raw_datum_typed(values[0],
										 state->input_kind,
										 exact_key_bytes,
										 state->exact_key_bytes,
										 &dimension,
	...
	memset(metadata_values, 0, sizeof(metadata_values));
	tq_collect_metadata_values(index,
							   values,
							   isnull,
							   metadata_values,
							   &metadata_nullmask,
							   state->metadata_descs,
							   state->metadata_attribute_count);
	...
	tq_buildstate_collect_vector(state,
								 ...
								 tid,
								 exact_key_bytes,
								 state->metadata_attribute_count > 0 ? metadata_values : NULL,
								 metadata_nullmask);
	pfree(exact_key_bytes);
}
...
tq_ambuild(Relation heap_relation, Relation index_relation, IndexInfo *index_info)
{
	...
	tq_validate_index_schema(index_relation);
	...
	build_state.metadata_attribute_count = tq_relation_metadata_attribute_count(index_relation,
																			  build_state.metadata_descs,
																			  error_buf,
																			  sizeof(error_buf));
	build_state.key_filter_attribute_count = tq_relation_filter_key_attribute_count(index_relation,
																			  error_buf,
																			  sizeof(error_buf));
	if (build_state.metadata_attribute_count == UINT16_MAX
...
tq_ambuildempty(Relation index_relation)
{
	...
	meta_fields.delta_batch_page_count = 0;
	meta_fields.exact_key_head_block = TQ_INVALID_BLOCK_NUMBER;
	meta_fields.exact_key_tail_block = TQ_INVALID_BLOCK_NUMBER;
	meta_fields.exact_key_page_count = 0;
			...
			IndexInfo *index_info)
{
	...
	if (metadata_attribute_count == UINT16_MAX)
		elog(ERROR, "%s", error_buf);
	tq_collect_metadata_values(index_relation,
							   values,
							   isnull,
							   metadata_values,
							   &metadata_nullmask,
	...
	if (meta_fields.dimension == 0)
	{
		...
		lane_config.reserve_bytes = TQ_PAGE_RESERVED_BYTES;
		lane_config.tid_bytes = TQ_TID_BYTES
			+ (int) tq_metadata_block_bytes(metadata_attribute_count)
			+ (int) sizeof(TqExactKeyRef);
		if (!tq_resolve_lane_count(&lane_config, &resolved_lane_count,
								   error_buf, sizeof(error_buf)))
			elog(ERROR, "%s", error_buf);
	...
	exact_key_len = tq_vector_storage_size(input_kind, meta_fields.dimension);
	exact_key_bytes = (uint8_t *) palloc(exact_key_len);
	...
	if (meta_fields.normalized)
		tq_maybe_normalize_fast_lane_vector(source_values,
											dimension,
											"embeddings");
	...
	if (meta_fields.list_count > 0)
	{
		tq_append_delta_packed_tuple_locked(index_relation,
											&meta_fields,
											&prod_config,
											list_id,
											&page_tid,
											...
											exact_key_bytes,
											exact_key_len,
											metadata_attribute_count > 0 ? metadata_values : NULL,
											metadata_attribute_count,
											metadata_nullmask,
											packed_code,
											prod_layout.total_bytes);
		needs_meta_write = true;
	}
	else
	{
				  ...
				  double *index_pages)
{
	...
	if (path->indexorderbys != NIL)
	{
		...
		if (root != NULL && path->indexclauses != NIL && path->indexinfo->rel != NULL)
		{
			qual_clauses = get_quals_from_indexclauses(path->indexclauses);
			qual_selectivity = clauselist_selectivity(root,
													 qual_clauses,
		...
		if (qual_selectivity <= 0.0 || !isfinite(qual_selectivity))
			qual_selectivity = 1.0 / Max(path->indexinfo->tuples, 1.0);
		if (qual_selectivity > 1.0)
			qual_selectivity = 1.0;

		if (output_rows <= 0.0 || !isfinite(output_rows))
			output_rows = clamp_row_est(path->indexinfo->tuples * qual_selectivity);
										  ...
										  &estimate))
		{
			...
			*index_selectivity = estimate.qual_selectivity * estimate.scanned_fraction;
					   ...
					   size_t packed_code_len)
{
	...
	TqMetaPageFields local_meta_fields;
	TqExactKeyRef stored_exact_key_ref;

	memset(&entry, 0, sizeof(entry));
	memset(&local_meta_fields, 0, sizeof(local_meta_fields));
	stored_exact_key_ref.block_number = TQ_INVALID_BLOCK_NUMBER;
	stored_exact_key_ref.entry_index = UINT16_MAX;

	if (exact_key_ref != NULL
		&& exact_key_ref->block_number != TQ_INVALID_BLOCK_NUMBER
		&& exact_key_ref->entry_index != UINT16_MAX)
		stored_exact_key_ref = *exact_key_ref;
	else
	{
		if (meta_fields == NULL || exact_key_bytes == NULL || exact_key_len == 0)
			elog(ERROR, "turboquant append requires either an exact-key ref or exact-key bytes");
		local_meta_fields = *meta_fields;
		tq_append_exact_key(index_relation,
							&local_meta_fields,
							exact_key_bytes,
							exact_key_len,
							&stored_exact_key_ref);
	}
	...
	if (!BufferIsValid(buffer) && have_entry)
	{
		...
		if (detached_block != TQ_INVALID_BLOCK_NUMBER)
		{
			...
			params.list_id = list_id;
			params.next_block = TQ_INVALID_BLOCK_NUMBER;
			params.metadata_attribute_count = metadata_value_count;
...
static void
tq_append_delta_packed_tuple_locked(Relation index_relation,
									TqMetaPageFields *meta_fields,
									const TqProdCodecConfig *prod_config,
									uint32_t list_id,
									const TqTid *heap_tid,
									...
									size_t packed_code_len)
{
	...
	{
		if (exact_key_bytes == NULL || exact_key_len == 0)
			elog(ERROR, "turboquant delta append requires either an exact-key ref or exact-key bytes");
		tq_append_exact_key(index_relation,
							meta_fields,
							exact_key_bytes,
							exact_key_len,
	...
	if (meta_fields->delta_tail_block != TQ_INVALID_BLOCK_NUMBER)
	{
		TqBatchPageHeaderView tail_header;
		bool		has_capacity = false;

		buffer = ReadBufferExtended(index_relation,
									MAIN_FORKNUM,
									meta_fields->delta_tail_block,
									RBM_NORMAL,
									NULL);
		memset(&tail_header, 0, sizeof(tail_header));
		if (!tq_batch_page_read_header(tq_page_payload(BufferGetPage(buffer)),
									   tq_page_payload_size(BufferGetPage(buffer)),
									   &tail_header,
									   error_buf,
									   sizeof(error_buf))
			|| !tq_batch_page_has_capacity(tq_page_payload(BufferGetPage(buffer)),
										   tq_page_payload_size(BufferGetPage(buffer)),
										   &has_capacity,
										   error_buf,
										   sizeof(error_buf)))
		...
		if (!has_capacity || tail_header.list_id != list_id)
		{
			ReleaseBuffer(buffer);
			buffer = InvalidBuffer;
		}
	...
	if (!BufferIsValid(buffer))
	{
		...
		buffer = ReadBufferExtended(index_relation,
									MAIN_FORKNUM,
									detached_block != TQ_INVALID_BLOCK_NUMBER ? detached_block : P_NEW,
									RBM_NORMAL,
									NULL);
		LockBuffer(buffer, BUFFER_LOCK_EXCLUSIVE);
		new_block = BufferGetBlockNumber(buffer);
		if (!tq_wal_init_batch_page(index_relation,
								   buffer,
								   &params,
								   error_buf,
								   sizeof(error_buf)))
		...
		if (meta_fields->delta_tail_block != TQ_INVALID_BLOCK_NUMBER)
			tq_update_batch_page_next_block(index_relation,
											meta_fields->delta_tail_block,
											new_block);
		if (meta_fields->delta_head_block == TQ_INVALID_BLOCK_NUMBER)
			meta_fields->delta_head_block = new_block;
		meta_fields->delta_tail_block = new_block;
		meta_fields->delta_batch_page_count += 1;
		LockBuffer(buffer, BUFFER_LOCK_UNLOCK);
	}

	LockBuffer(buffer, BUFFER_LOCK_EXCLUSIVE);
	if (tq_batch_page_is_soa(tq_page_payload(BufferGetPage(buffer)),
							 tq_page_payload_size(BufferGetPage(buffer))))
	{
		uint8_t	   *nibbles;
		float		gamma = 0.0f;
		...
		nibbles = (uint8_t *) palloc(prod_config->dimension);
		if (!tq_prod_extract_nibbles(prod_config,
									 packed_code,
									 packed_code_len,
									 nibbles,
									 prod_config->dimension,
									 error_buf,
									 sizeof(error_buf))
			|| !tq_prod_read_gamma(prod_config,
								   packed_code,
								   packed_code_len,
								   &gamma,
								   error_buf,
								   sizeof(error_buf))
			|| !tq_wal_append_batch_soa(index_relation,
										buffer,
										heap_tid,
										&stored_exact_key_ref,
										metadata_values,
	...
	{
		if (!tq_wal_append_batch_code(index_relation,
									  buffer,
									  heap_tid,
									  &stored_exact_key_ref,
									  metadata_values,
	...
	meta_fields->delta_live_count += 1;
	LockBuffer(buffer, BUFFER_LOCK_UNLOCK);
	ReleaseBuffer(buffer);
}
				   ...
				   TqVacuumSummary *summary)
{
	...
	if (meta_fields->list_count == 0)
	{
		...
		for (block_number = 1; block_number < nblocks; block_number++)
		{
			...
			if (!tq_page_read_kind(tq_page_payload(page), tq_page_payload_size(page),
								   &page_kind, error_buf, sizeof(error_buf)))
			{
				LockBuffer(buffer, BUFFER_LOCK_UNLOCK);
				ReleaseBuffer(buffer);
				elog(ERROR, "%s", error_buf);
...
bool
tq_compact_index_relation(Relation index_relation,
						  char *errmsg,
						  size_t errmsg_len)
{
	TqMetaPageFields meta_fields;
	...
	{
		snprintf(errmsg, errmsg_len,
				 "invalid turboquant maintenance: relation must be non-null");
		return false;
	}

	memset(&meta_fields, 0, sizeof(meta_fields));
	memset(&summary, 0, sizeof(summary));
	if (!tq_read_meta_page(index_relation, &meta_fields, errmsg, errmsg_len))
		return false;

	tq_summarize_index(index_relation, &meta_fields, NULL, NULL, false, &summary);
	return true;
}

bool
tq_merge_delta_relation(Relation index_relation,
						...
						size_t errmsg_len)
{
	...
	uint8_t	   *packed_code = NULL;
	uint8_t	   *lane_nibbles = NULL;
	bool	   *rewritten_lists = NULL;
	BlockNumber block_number = TQ_INVALID_BLOCK_NUMBER;
	uint16_t	metadata_attribute_count = 0;
	char		error_buf[256];

	if (merged_delta_count == NULL
		|| rewritten_list_count == NULL
		|| recycled_delta_page_count == NULL)
	{
		snprintf(errmsg, errmsg_len,
				 "invalid turboquant maintenance: merge outputs must be non-null");
		return false;
	}

	*merged_delta_count = 0;
	*rewritten_list_count = 0;
	*recycled_delta_page_count = 0;
	memset(&meta_fields, 0, sizeof(meta_fields));
	memset(&prod_config, 0, sizeof(prod_config));
	memset(&prod_layout, 0, sizeof(prod_layout));
	memset(metadata_descs, 0, sizeof(metadata_descs));
	...
	if (metadata_attribute_count == UINT16_MAX)
	{
		snprintf(errmsg, errmsg_len, "%s", error_buf);
		return false;
	}
	...
	packed_code = (uint8_t *) palloc(prod_layout.total_bytes);
	lane_nibbles = (uint8_t *) palloc(Max((uint32_t) 1, prod_config.dimension));
	if (meta_fields.list_count > 0)
		rewritten_lists = (bool *) palloc0(sizeof(bool) * (size_t) meta_fields.list_count);

	block_number = meta_fields.delta_head_block;
	while (block_number != TQ_INVALID_BLOCK_NUMBER)
	{
		Buffer		buffer;
		Page		page;
		...
		BlockNumber next_block = TQ_INVALID_BLOCK_NUMBER;
		uint16_t	lane = 0;
		bool		page_is_soa = false;
		uint16_t	page_metadata_attribute_count = 0;
		const uint8_t *nibble_ptr = NULL;
		const float *gamma_ptr = NULL;
		uint32_t	soa_dimension = 0;
		uint16_t	soa_lane_count = 0;
		...
		page = BufferGetPage(buffer);
		memset(&header, 0, sizeof(header));
		if (!tq_batch_page_read_header(tq_page_payload(page),
									   tq_page_payload_size(page),
									   &header,
									   error_buf,
									   sizeof(error_buf))
			|| !tq_batch_page_get_metadata_attribute_count(tq_page_payload(page),
														  tq_page_payload_size(page),
		...
		next_block = header.next_block;
		page_is_soa = tq_batch_page_is_soa(tq_page_payload(page),
										   tq_page_payload_size(page));
		if (page_is_soa
			&& (!tq_batch_page_get_nibble_ptr(tq_page_payload(page),
											  tq_page_payload_size(page),
											  &nibble_ptr,
											  &soa_dimension,
											  &soa_lane_count,
											  error_buf,
											  sizeof(error_buf))
				|| !tq_batch_page_get_gamma_ptr(tq_page_payload(page),
												tq_page_payload_size(page),
												&gamma_ptr,
												error_buf,
												sizeof(error_buf))))
		...
		if (tq_batch_page_next_live_lane(tq_page_payload(page),
										 tq_page_payload_size(page),
										 -1,
										 &lane,
										 error_buf,
										 sizeof(error_buf)))
		{
			do
			{
				...
				exact_key_ref.entry_index = UINT16_MAX;
				memset(metadata_values, 0, sizeof(metadata_values));
				if (!tq_batch_page_get_tid(tq_page_payload(page),
										   tq_page_payload_size(page),
										   lane,
										   &tid,
										   error_buf,
				...
				if (!tq_batch_page_get_exact_key_ref(tq_page_payload(page),
													 tq_page_payload_size(page),
													 lane,
													 &exact_key_ref,
													 error_buf,
				...
				if (page_metadata_attribute_count > 0
					&& !tq_batch_page_get_metadata_block(tq_page_payload(page),
														 tq_page_payload_size(page),
														 lane,
														 metadata_values,
														 page_metadata_attribute_count,
				...
				if (page_is_soa)
				{
					...
					size_t byte_col = (size_t) lane / 2u;
					bool is_high = (lane % 2u) != 0;
					uint32_t d = 0;

					for (d = 0; d < soa_dimension; d++)
					{
						size_t off = (size_t) d * pair_cols + byte_col;
						uint8_t packed_nibble = nibble_ptr[off];

						lane_nibbles[d] = is_high
							? (uint8_t) ((packed_nibble >> 4u) & 0x0Fu)
							: (uint8_t) (packed_nibble & 0x0Fu);
					}
					if (!tq_prod_nibbles_gamma_to_packed(&prod_config,
														 lane_nibbles,
														 soa_dimension,
														 gamma_ptr[lane],
														 packed_code,
				...
				else if (!tq_batch_page_get_code(tq_page_payload(page),
												 tq_page_payload_size(page),
												 lane,
												 packed_code,
												 prod_layout.total_bytes,
				...
				tq_append_packed_tuple(index_relation,
									   &meta_fields,
									   &prod_config,
									   header.list_id,
									   &tid,
									   ...
									   packed_code,
									   prod_layout.total_bytes);
				*merged_delta_count += 1;
				if (rewritten_lists != NULL && header.list_id < meta_fields.list_count)
					rewritten_lists[header.list_id] = true;
			}
			while (tq_batch_page_next_live_lane(tq_page_payload(page),
												tq_page_payload_size(page),
												(int) lane,
												&lane,
												error_buf,
...

		{
			...
			params.list_id = TQ_DETACHED_FREE_LIST_ID;
			params.next_block = TQ_INVALID_BLOCK_NUMBER;
			params.metadata_attribute_count = page_metadata_attribute_count;
			if (page_is_soa)
				params.dimension = prod_config.dimension;
			if (!tq_wal_init_batch_page(index_relation,
										buffer,
	...
	if (rewritten_lists != NULL)
	{
		uint32_t list_id = 0;

		for (list_id = 0; list_id < meta_fields.list_count; list_id++)
		{
			if (rewritten_lists[list_id])
				*rewritten_list_count += 1;
		}
		pfree(rewritten_lists);
	}
	pfree(packed_code);
	pfree(lane_nibbles);
	return true;
					...
					size_t errmsg_len)
{
	...
	if (!tq_batch_page_scan_prod_with_scratch_filtered(tq_page_payload(page),
													   ...
													   opaque->metadata_attribute_count,
													   heap,
													   shadow_decode_heap,
													   &opaque->scan_scratch,
													   errmsg,
								  ...
								  size_t errmsg_len)
{
		   ...
		   && summarized_pages < (size_t) entry->batch_page_count)
	{
		...
		{
			TqBatchPageSummary summary;
			bool summary_matches_filters = true;
			...
			if (!tq_scan_summary_matches_filters(&summary,
												 opaque->filter_clauses,
												 opaque->filter_clause_count,
...
static bool
tq_estimate_filtered_pages_for_list(Relation index_relation,
									 const TqListDirEntry *entry,
									 TqScanOpaque *opaque,
									 uint32_t *estimated_live_count,
									 uint32_t *estimated_page_count,
									 ...
									 size_t errmsg_len)
{
	...
	if (estimated_live_count == NULL || estimated_page_count == NULL)
	{
		snprintf(errmsg, errmsg_len,
				 "invalid turboquant scan: filtered page estimate requires output counters");
		return false;
	}

	*estimated_live_count = entry != NULL ? entry->live_count : 0;
	*estimated_page_count = entry != NULL ? entry->batch_page_count : 0;

	if (entry == NULL || opaque == NULL)
	{
		snprintf(errmsg, errmsg_len,
				 "invalid turboquant scan: filtered page estimate requires entry and scan state");
		return false;
	}

	if (opaque->filter_clause_count == 0
	...
	if (summarized_pages == 0 || matched_pages == 0)
	{
		*estimated_live_count = 0;
		*estimated_page_count = 0;
		return true;
	}

	*estimated_page_count = (uint32_t) Min(matched_pages, (size_t) entry->batch_page_count);
	*estimated_live_count = (uint32_t) Min((size_t) entry->live_count,
										   ((size_t) entry->live_count * matched_pages
											+ (size_t) entry->batch_page_count - 1)
										   / (size_t) entry->batch_page_count);
	return true;
}
...
tq_scan_prepare(IndexScanDesc scan, TqScanOpaque *opaque)
{
	...
	metadata_attribute_count = tq_relation_metadata_attribute_count(scan->indexRelation,
																   opaque->metadata_descs,
																   error_buf,
																   sizeof(error_buf));
	...
	if (scan->numberOfKeys > (int) key_filter_attribute_count)
		ereport(ERROR,
				(errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
				 errmsg("turboquant ordered scans support equality, IS NULL, or ANY filters on compact fixed-width key columns")));

	opaque->metadata_attribute_count = metadata_attribute_count;
	...
	for (key_index = 0; key_index < scan->numberOfKeys; key_index++)
	{
		...
		TqMetadataFilterClause *clause = NULL;
		TqMetadataAttrDesc *desc = NULL;

		if (filter_key->sk_attno < 2
			|| filter_key->sk_attno > key_filter_attribute_count + 1
			|| (((filter_key->sk_flags & SK_SEARCHNULL) == 0)
				&& filter_key->sk_strategy != TQ_FILTER_EQ_STRATEGY)
			|| (filter_key->sk_flags & SK_SEARCHNOTNULL) != 0)
			ereport(ERROR,
					(errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
					 errmsg("turboquant ordered scans only support equality, IS NULL, or ANY filters on compact key columns")));

		clause = &opaque->filter_clauses[opaque->filter_clause_count++];
		clause->attribute_index = (uint16_t) (filter_key->sk_attno - 2);
		desc = &opaque->metadata_descs[clause->attribute_index];
		clause->kind = desc->kind;
		clause->match_null = false;
		clause->value_count = 0;

		if ((filter_key->sk_flags & SK_SEARCHNULL) != 0)
		{
			clause->match_null = true;
			continue;
		}
		if ((filter_key->sk_flags & SK_SEARCHARRAY) != 0)
		{
			...
			char		element_align = TYPALIGN_INT;

			if (ARR_ELEMTYPE(array_value) != desc->typid)
				ereport(ERROR,
						(errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
						 errmsg("turboquant ANY filters require arrays matching the filter column type")));

			get_typlenbyvalalign(desc->typid,
								 &element_len,
								 &element_byval,
								 &element_align);
			...
			if (element_count > TQ_MAX_FILTER_VALUES_PER_CLAUSE)
				ereport(ERROR,
						(errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
						 errmsg("turboquant ANY filters support at most %d values per clause",
			...
			for (element_index = 0; element_index < element_count; element_index++)
			{
				if (nulls[element_index])
					continue;
				if (!tq_metadata_encode_datum(desc->kind,
											  elements[element_index],
											  clause->values[clause->value_count],
											  error_buf,
											  sizeof(error_buf)))
					elog(ERROR, "%s", error_buf);
				clause->value_count++;
	...
	opaque->source_dimension = meta_fields.dimension;
	opaque->exact_key_bytes = tq_vector_storage_size(opaque->input_kind,
	...
	if (meta_fields.normalized)
		tq_maybe_normalize_fast_lane_vector(query_values,
											query_dimension,
											"query vectors");
	...
	if (meta_fields.list_count == 0)
	{
		...
		if (false)
		{
			...
			for (block_number = 1; block_number < nblocks; block_number++)
			{
				...
				tq_scan_stats_record_bound_data_page_read();
				memset(&page_kind, 0, sizeof(page_kind));
				if (!tq_page_read_kind(tq_page_payload(BufferGetPage(buffer)),
									   tq_page_payload_size(BufferGetPage(buffer)),
									   &page_kind, error_buf, sizeof(error_buf)))
				{
					ReleaseBuffer(buffer);
	...
	else
	{
		...
		if (opaque->filter_clause_count > 0)
		{
			for (index = 0; index < (uint32_t) ranked_count; index++)
			{
				uint32_t estimated_live_count = ranked_live_counts[index];
				uint32_t estimated_page_count = ranked_page_counts[index];

				if (!tq_estimate_filtered_pages_for_list(scan->indexRelation,
														 &selected_entries[index],
														 opaque,
														 &estimated_live_count,
				...
				ranked_live_counts[index] = estimated_live_count;
				ranked_page_counts[index] = estimated_page_count;
			}
		}
		...
		total_live += (size_t) meta_fields.delta_live_count;
		...
		if (iterative_filtered_completion)
		{
			...
			for (index = 0; index < ranked_count; index++)
			{
				...
				if (page_count > list_page_start + 1)
				{
					qsort(page_candidates + list_page_start,
						  page_count - list_page_start,
						  sizeof(page_candidates[0]),
						  tq_bounded_page_candidate_compare);
				}
		...
		delta_page_start = page_count;
		if (meta_fields.delta_head_block != TQ_INVALID_BLOCK_NUMBER)
		{
			if (!tq_collect_live_pages_for_chain(scan->indexRelation,
			...
			for (index = (uint32_t) delta_page_start; index < page_count; index++)
			{
				if (!tq_scan_batch_block(scan->indexRelation,
										 page_candidates[index].block_number,
										 opaque,
...
tq_amcanreturn(Relation index_relation, int attno)
{
	...
	if (attno <= 0 || attno > IndexRelationGetNumberOfAttributes(index_relation))
		return false;

	atttypid = TupleDescAttr(index_relation->rd_att, attno - 1)->atttypid;
	if (attno == 1)
		return tq_vector_input_kind_from_typid(atttypid,
											   &input_kind,
											   NULL,
											   0);

	return tq_metadata_kind_from_typid(atttypid,
									   &kind,
									   NULL,
									   0);
}
...
tq_amgettuple(IndexScanDesc scan, ScanDirection direction)
{
	...
	if (scan->xs_want_itup)
	{
		...
		if (opaque->exact_key_bytes > 0)
		{
			char error_buf[256];

			exact_key_bytes = (uint8_t *) palloc(opaque->exact_key_bytes);
			if (!tq_read_exact_key_bytes(scan->indexRelation,
										 &entry.exact_key_ref,
										 exact_key_bytes,
										 opaque->exact_key_bytes,
										 error_buf,
										 sizeof(error_buf)))
				elog(ERROR, "%s", error_buf);
			if (!tq_vector_datum_from_raw_bytes_typed(exact_key_bytes,
													  opaque->exact_key_bytes,
													  opaque->input_kind,
													  opaque->source_dimension,
													  &values[0],
													  error_buf,
													  sizeof(error_buf)))
				elog(ERROR, "%s", error_buf);
		...
		for (attno = 1; attno < attribute_count; attno++)
		{
			...
			memset(&desc, 0, sizeof(desc));
			if (metadata_index >= entry.metadata_attribute_count)
				continue;
			if (!tq_metadata_attr_desc_init(TupleDescAttr(tupdesc, attno)->atttypid,
											&desc,
											NULL,
											0))
				continue;
			if ((entry.metadata_nullmask & (uint16_t) (1u << metadata_index)) != 0)
				continue;
			if (tq_metadata_decode_datum(desc.kind,
										 entry.metadata_values
										 + ((size_t) metadata_index * TQ_METADATA_SLOT_BYTES),
										 &values[attno],
										 NULL,
										 0))
			{
		...
		for (attno = 0; attno < attribute_count; attno++)
		{
			Form_pg_attribute attribute = TupleDescAttr(tupdesc, attno);

			if (!isnull[attno] && !attribute->attbyval)
				pfree(DatumGetPointer(values[attno]));
		}
		if (exact_key_bytes != NULL)
			pfree(exact_key_bytes);
		pfree(values);
